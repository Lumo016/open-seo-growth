...
function setExportReady(isReady) {
  ["copyReportBtn", "downloadMarkdownBtn", "downloadJsonBtn"].forEach((id) => {
    const button = $(id);
    if (button) button.disabled = !isReady;
  });
  if ($("reportExportSummary")) {
    $("reportExportSummary").textContent = isReady
      ? "Export a client-ready Markdown brief or raw JSON evidence from this browser session. No account or Google connection is required."
      : "Run an audit to unlock a Markdown brief, raw JSON evidence, and a copyable client summary.";
  }
}
